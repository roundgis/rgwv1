...
class ZbDevice:
    ...
    IDX1 = """
    create index if not exists rgw_zb_device_idx1 on rgw_zb_device(nid)
    """

    IDX2 = """
    create index if not exists rgw_zb_device_idx2 on rgw_zb_device(moduleid)
    """
    ...
    def SqlRows_Remove(cls, deviceids):
        ...
        if isinstance(deviceids, list):
            for devid in deviceids:
                sqls.append(["delete from rgw_zb_device where id=?", (devid,)])
        else:
            sqls.append(["delete from rgw_zb_device where id=?", (deviceids,)])
        return sqls
    ...
    def DynUpdate(cls, mdl):
        ...
        for key in mdl:
            if key not in ('id',):
                terms.append("{0}=?".format(key))
                args.append(mdl[key])
        update_sql = "update rgw_zb_device set "
        update_sql += ",".join(terms)
        update_sql += " where id=?"
        args.append(mdl['id'])
        return [update_sql, args]
    ...
    def DynInsert(cls, rec_tbl, ignored=False):
        ...
        for key in rec_tbl:
            ...
            args.append(rec_tbl[key])
        if ignored:
            insert_sql = "insert or ignore into rgw_zb_device("
        else:
            insert_sql = "insert into rgw_zb_device("
        insert_sql += ",".join(terms)
        insert_sql += ") values ("
        insert_sql += ",".join(marks)
        insert_sql += ")"
...
class ZbModule:
    ...
    def SqlRows_Remove(cls, ids):
        sqls = []
        for devid in ids:
            sqls.append(["delete from rgw_zb_module where id=?", (devid,)])
        return sqls
    ...
    def DynUpdate(cls, mdl, ignored=False):
        ...
        if ignored:
            update_sql = "update or ignore rgw_zb_module set "
        else:
            update_sql = "update rgw_zb_module set "
        update_sql += ",".join(terms)
        update_sql += " where id=?"
        args.append(mdl['id'])
        return [update_sql, args]
    ...
    def DynInsert(cls, mdl, ignored=False):
        ...
        if cls.HasBackupData(mdl):
            ...
            args.append(json.dumps(mdl['backup_data']))
        if ignored:
            insert_sql = "insert or ignore into rgw_zb_module("
        else:
            insert_sql = "insert into rgw_zb_module("
        insert_sql += ",".join(terms)
        insert_sql += ") values ("
        insert_sql += ",".join(marks)
        insert_sql += ")"
...
class DeviceOpLog:
    ...
    IDX1 = """create index if not exists rgw_device_op_log_idx1 on rgw_device_op_log(cts)"""
    ...
    def DynInsert(cls, rec_tbl, ignored=False):
        ...
        for key in rec_tbl:
            ...
            args.append(rec_tbl[key])
        if ignored:
            insert_sql = "insert or ignore into rgw_device_op_log("
        else:
            insert_sql = "insert into rgw_device_op_log("
        insert_sql += ",".join(terms)
        insert_sql += ") values ("
        insert_sql += ",".join(marks)
        insert_sql += ")"
